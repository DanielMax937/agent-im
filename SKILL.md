...
- **Runtime**: `claude` (default), `codex`, `cursor`, `auto`
  ...
  - `cursor` — uses Cursor agent CLI via Codex SDK protocol (requires `agent` CLI; auth via `OPENAI_API_KEY` or `CTI_CURSOR_API_KEY`)
