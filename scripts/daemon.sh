  ...
  if [ "$mode" = "strict" ]; then
    case "$runtime" in
      ...
      claude)
        ...
        # Strip OPENAI_* and CURSOR_* to avoid cross-runtime leakage.
        while IFS='=' read -r name _; do
          case "$name" in OPENAI_*|CURSOR_*) unset "$name" 2>/dev/null || true ;; esac
        done < <(env)
        ;;
      auto)
        # Keep ANTHROPIC_*, OPENAI_*, and CURSOR_* for auto mode
        ;;
    esac
  fi
}
