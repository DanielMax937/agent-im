...
# --- Cursor checks (cursor mode) ---
if [ "$CTI_RUNTIME" = "cursor" ]; then
  if command -v agent &>/dev/null; then
    CURSOR_VER=$(agent --version 2>/dev/null || echo "unknown")
    check "Cursor agent CLI available (${CURSOR_VER})" 0
  else
    check "Cursor agent CLI available (not found in PATH)" 1
  fi

  # Cursor reuses @openai/codex-sdk
  CODEX_SDK="$SKILL_DIR/node_modules/@openai/codex-sdk"
  if [ -d "$CODEX_SDK" ]; then
    check "@openai/codex-sdk installed (required by Cursor provider)" 0
  else
    check "@openai/codex-sdk installed (not found — run 'npm install' in $SKILL_DIR)" 1
  fi

  # Check Cursor auth: CTI_CURSOR_API_KEY / CURSOR_API_KEY / OPENAI_API_KEY
  CURSOR_AUTH=1
  if [ -n "${CTI_CURSOR_API_KEY:-}" ] || [ -n "${CURSOR_API_KEY:-}" ] || [ -n "${OPENAI_API_KEY:-}" ]; then
    CURSOR_AUTH=0
  fi
  if [ "$CURSOR_AUTH" = "0" ]; then
    check "Cursor auth available (API key)" 0
  else
    check "Cursor auth available (set CTI_CURSOR_API_KEY or OPENAI_API_KEY)" 1
  fi
fi
...
# --- Proxy configuration ---
CTI_PROXY=$(get_config CTI_PROXY)
CTI_PROXY="${CTI_PROXY:-${HTTPS_PROXY:-${HTTP_PROXY:-${ALL_PROXY:-}}}}"
CURL_PROXY_OPTS=""
if [ -n "$CTI_PROXY" ]; then
  echo "Proxy: $CTI_PROXY"
  CURL_PROXY_OPTS="-x $CTI_PROXY"
  # Verify proxy is reachable by hitting a lightweight endpoint
  PROXY_TEST=$(curl -s --max-time 5 $CURL_PROXY_OPTS "https://api.telegram.org/" 2>/dev/null || echo "FAIL")
  if [ "$PROXY_TEST" != "FAIL" ]; then
    check "Proxy reachable (api.telegram.org via $CTI_PROXY)" 0
  else
    check "Proxy reachable (api.telegram.org via $CTI_PROXY — connection failed)" 1
  fi
else
  echo "Proxy: none"
fi
...
if [ -f "$CONFIG_FILE" ]; then
  ...
  if echo "$CTI_CHANNELS" | grep -q telegram; then
    ...
    if [ -n "$TG_TOKEN" ]; then
      TG_RESULT=$(curl -s --max-time 5 $CURL_PROXY_OPTS "https://api.telegram.org/bot${TG_TOKEN}/getMe" 2>/dev/null || echo '{"ok":false}')
  ...
  if echo "$CTI_CHANNELS" | grep -q feishu; then
    ...
    if [ -n "$FS_APP_ID" ] && [ -n "$FS_SECRET" ]; then
      FEISHU_RESULT=$(curl -s --max-time 5 $CURL_PROXY_OPTS -X POST "${FS_DOMAIN}/open-apis/auth/v3/tenant_access_token/internal" \
  ...
  if echo "$CTI_CHANNELS" | grep -q qq; then
    ...
    if [ -n "$QQ_APP_ID" ] && [ -n "$QQ_APP_SECRET" ]; then
      QQ_TOKEN_RESULT=$(curl -s --max-time 10 $CURL_PROXY_OPTS -X POST "https://bots.qq.com/app/getAppAccessToken" \
        -H "Content-Type: application/json" \
      ...
      if [ -n "$QQ_ACCESS_TOKEN" ]; then
        ...
        QQ_GW_RESULT=$(curl -s --max-time 10 $CURL_PROXY_OPTS "https://api.sgroup.qq.com/gateway" \
