...
describe('OpenAI chat completions parsing', () => {
  ...
  it('keeps JSON-looking text payloads as assistant text in non-streaming collection', async () => {
    const providerStream = new ReadableStream<string>({
      start(controller) {
        controller.enqueue(`data: ${JSON.stringify({
          type: 'text',
    ...
    assert.equal(completion.sessionId, 'thread-json');
  });

  it('keeps JSON-looking text payloads as streamed assistant text', async () => {
    const providerStream = new ReadableStream<string>({
      start(controller) {
        controller.enqueue(`data: ${JSON.stringify({
          type: 'text',
    ...
    const contentChunk = payloads.find((payload) => payload.choices?.[0]?.delta?.content);
    assert.equal(contentChunk.choices[0].delta.content, '{"ok":true}');
  });
});
